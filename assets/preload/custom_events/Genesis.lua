...
function onUpdate()
    ...
    elseif changeNotes == 1 then
        for notesLength = 0,getProperty('notes.length')-1 do
            if getPropertyFromGroup('notes',notesLength,'noteType') == '' then
                ...
                if getPropertyFromGroup('notes',notesLength,'isSustainNote') == true then
                    setPropertyFromGroup('notes', notesLength,'scale.y',20)
                end
            end
        end
        changeNotes = 0
    ...
    if ChangedCharacter == false then
        ...
        dadX = getProperty('boyfriend.x')
        dadY = getProperty('boyfriend.y')
...
function onEvent(name,v1)
    if name == 'Genesis' then
        if string.lower(v1) ~= 'false' then
            ...
            setProperty('boyfriend.x',845 +  getProperty('boyfriend.positionArray[0]'))
            setProperty('boyfriend.y',-400 + getProperty('boyfriend.positionArray[1]'))

            setProperty('dad.x',100 +  getProperty('dad.positionArray[0]'))
            setProperty('dad.y',-400 + getProperty('dad.positionArray[1]'))
            triggerEvent('static','true','')
            runTimer('changeNotesSonic',0.2)
            setProperty('gf.x',300 + getProperty('gf.positionArray[0]'))
            setProperty('gf.y',-350 + getProperty('gf.positionArray[1]'))
        else
